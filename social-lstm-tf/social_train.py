...
def main():
    ...
    parser.add_argument('--maxNumPeds', type=int, default=3,
                        help='Maximum Number of Pedestrians/hands')
...
def train(args):
    ...
    with open(os.path.join('save/my_socialmodel', 'mysocial_config.pkl'), 'wb') as f:
        pickle.dump(args, f)
    ...
    with tf.Session() as sess:
        ...
        for e in range(args.num_epochs):
            ...
            for b in range(data_loader.num_batches):
                ...
                # save_txt = open('save/my_socialmodel/loss.txt', 'w')
                # epochs = args.num_epochs * data_loader.num_batches
                # for i in range(epochs):
                #     save_txt.write("step = {}, train_loss = {:.3f}" .format(
                #         i, loss_batch) + '\n')
                #     i += 1
                # Save the model if the current epoch and batch number match the frequency
                if (e * data_loader.num_batches + b) % args.save_every == 0 and ((e * data_loader.num_batches + b) > 0):
                    checkpoint_path = os.path.join('save/my_socialmodel', 'mysocial_model.ckpt')
